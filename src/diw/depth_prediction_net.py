...
def encoder_resnet(target_image, weight_reg, is_training, normalizer_fn=None):
    ...
    # conv2_x
    x = s_residual_block(x, is_training, name="conv2_1", normalizer_fn=normalizer_fn)
...
def depth_prediction_resnet18unet(
    images,
    is_training,
    decoder_weight_reg=0.0,
    normalizer_fn=None,
    ...
    bottleneck, skip_connections = encoder_resnet(
        images, weight_reg=None, is_training=is_training, normalizer_fn=normalizer_fn
    )
...
def s_residual_block_first(
    x, is_training, out_channel, strides, name="unit", normalizer_fn=None
):
        ...
        if in_channel == out_channel:
            if strides == 1:
                ...
                shortcut = tf.nn.max_pool(
                    x, [1, strides, strides, 1], [1, strides, strides, 1], "VALID"
                )
        else:
...
def s_residual_block(
    x, is_training, input_q=None, output_q=None, name="unit", normalizer_fn=None
):
    """Helper function for defining ResNet architecture."""
    normalizer_fn = normalizer_fn or _bn
