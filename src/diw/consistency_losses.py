...
def rgbd_consistency_loss(frame1transformed_depth, frame1rgb, frame2depth, frame2rgb):
    ...
    frame2rgb_resampled = contrib_resampler.resampler(frame2rgb, pixel_xy)
    ...
    rgb_error = tf.abs(frame2rgb_resampled - frame1rgb) * tf.expand_dims(
    ...
    depth_proximity_weight = (
        ...
        * tf.to_float(frame1transformed_depth.mask)
    )
...
def motion_field_consistency_loss(
    frame1transformed_pixelxy, mask, rotation1, translation1, rotation2, translation2
):
    ...
    translation_error = tf.reduce_mean(
        mask * norm(trans_zero) / (1e-24 + norm(translation1) + norm(translation2))
    )
...
def rgbd_and_motion_consistency_loss(
    ...
    endpoints = rgbd_consistency_loss(
        frame1transformed_depth, frame1rgb, frame2depth, frame2rgb
    )
    ...
    endpoints.update(
        motion_field_consistency_loss(
            frame1transformed_depth.pixel_xy,
