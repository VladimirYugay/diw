...
class DataReader(object):
    ...
    def read_data(self):
        ...
        with tf.name_scope("data_loading"):
            ...
            if self.imagenet_norm:
                ...
                im_sd = tf.tile(tf.constant(IMAGENET_SD), multiples=[self.seq_length])
    ...
    def augment_image_colorspace(cls, image_stack):
        ...
        apply_brightness = tf.less(
            tf.random_uniform(shape=[], minval=0.0, maxval=1.0, dtype=tf.float32), 0.5
        )
    ...
    @classmethod
    def augment_images_flip(cls, image_stack, seg_stack, intrinsics, randomized=True):
        ...
        def flip(cls, image_stack, seg_stack, intrinsics):
            ...
            return (
                tf.image.flip_left_right(image_stack),
    ...
    def compile_file_list(self, data_dir, split, load_pose=False):
        ...
        image_file_list = [
            os.path.join(
                data_dir, subfolders[i], frame_ids[i] + "." + self.file_extension
            )
            for i in range(len(frames))
        ]
        segment_file_list = [
            os.path.join(
                data_dir, subfolders[i], frame_ids[i] + "-fseg." + self.file_extension
            )
            for i in range(len(frames))
        ]
        cam_file_list = [
