...
class TransformedDepthMap(object):
    ...
    def pixel_xy(self):
        if self._pixel_xy is None:
            ...
            self._pixel_xy = tf.stack(
                [self._pixel_x, self._pixel_y], axis=3, name="%s/pixel_xy" % name
            )
        return self._pixel_xy


def using_motion_vector(
    depth, translation, rotation_angles, intrinsic_mat, distortion_coeff=None, name=None
):
    ...
    with tf.name_scope(
        name,
        "TransformDepthMapUsingMotionVector",
...
def _using_motion_vector_with_distortion(
    depth, translation, rotation_angles, intrinsic_mat, distortion_coeff=0.0
):
    """A helper for using_motion_vector. See docstring therein."""
    ...
    undistortion_factor = tf.stack(
        [undistortion_factor, undistortion_factor, tf.ones_like(undistortion_factor)],
        axis=1,
    )
    ...
    pcoords = tf.einsum(
        "bij,bjhw->bihw", intrinsic_mat, tf.stack([x, y, tf.ones_like(x)], axis=1)
    )
