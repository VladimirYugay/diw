...
from diw import (
    consistency_losses,
    depth_prediction_net,
    motion_prediction_net,
    randomized_layer_normalization,
    reader,
    transform_depth_map,
    transform_utils,
)
...
class Model(object):
    ...
    def _build_train_op(self):
        with tf.name_scope("train_op"):
            ...
            self.train_op = slim.learning.create_train_op(
                self.total_loss, optim, clip_gradient_norm=10.0
            )
    ...
    def _build_summaries(self):
        ...
        for i in range(SEQ_LENGTH):
            tf.summary.image(
                "image%d" % i, self.image_stack[:, :, :, 3 * i : 3 * (i + 1)]
            )
            if i in self.depth:
    ...
    def _build_depth_test_graph(self):
        ...
        with tf.variable_scope(DEPTH_SCOPE, reuse=tf.AUTO_REUSE):
            ...
            def _normalizer_fn(x, is_train, name="bn"):
                return randomized_layer_normalization.normalize(x, is_train, name, None)
    ...
    def _build_egomotion_test_graph(self):
        ...
        with tf.variable_scope(tf.get_variable_scope(), reuse=tf.AUTO_REUSE):
            ...
            with tf.name_scope("compute_loss"):
                ...
                inv_rot, inv_trans, _, _ = motion_prediction_net.motion_field_net(
            ...
            self.trans = 0.5 * (
                -tf.squeeze(tf.matmul(self.rot, tf.expand_dims(inv_trans, -1)), axis=-1)
                + trans
            )
    ...
    def inference_egomotion(self, image1, image2, sess):
        return sess.run(
            [self.rot, self.trans],
            feed_dict={self._image1: image1, self._image2: image2},
        )
...
def _smoothness(motion_map):
    norm = tf.reduce_mean(tf.square(motion_map), axis=[1, 2, 3], keep_dims=True) * 3.0
