...
def main(data_dir, checkpoint_dir, output_dir, save_img):
    if save_img:
        plt.figure()
    ...
    with tf.Session() as sess:
        ...
        for seq_path in sequence_paths:
            ...
            img_paths = sorted(
                [p for p in (seq_path / "rgb").glob("*") if p.is_file()],
            ...
            for img_path in img_paths:
                ...
                np.save(str(depth_path / img_name), depth)
                if save_img:
                    plt.imshow(depth, plt.cm.get_cmap("plasma").reversed())
                    plt.savefig(str(depth_img_path / (img_name + ".png")))
                    plt.clf()
